...
(function() {
    ...
    for (let i = 0; i < vendors.length && !globalThis.requestAnimationFrame; ++i) {
        const vp = vendors[i];
        globalThis.requestAnimationFrame = globalThis[vp+'RequestAnimationFrame'];
        globalThis.cancelAnimationFrame = (globalThis[vp+'CancelAnimationFrame'] || window[vp+'CancelRequestAnimationFrame']);
    }
    if (/iP(ad|hone|od).*OS 6/.test(globalThis.navigator.userAgent)
...
function Game(id, params) {
    ...
    Map.prototype.finder = function(params) {
        ...
        const steps = new Array(y_length).fill(0).map(() => new Array(x_length).fill(0));
        const _getValue = function(x, y) {
            if (options.map[y] && options.map[y][x] !== undefined) {
                return options.map[y][x];
            }
            return -1;
        };
        const _next = function(to) {
    ...
    Stage.prototype.createMap = function(options) {
        ...
        map.data = structuredClone(map._params.data);
    ...
    Stage.prototype.bind = function(eventType, callback) {
        if (!_events[eventType]) {
            _events[eventType] = {};
            globalThis.addEventListener(eventType, function(e) {
                const key = 's' + _index;
                if (_events[eventType][key]) {
                    _events[eventType][key](e);
                }
