(function () {
	...
	const Player = Mario.Player = function (pos) {
	...
	Player.prototype.shoot = function () {
		...
		this.fireballs += 1;
		const fb = new Mario.Fireball([this.pos[0] + 8, this.pos[1]]); //I hate you, Javascript.
		fb.spawn(this.left);
		this.shooting = 2;
	}
	...
	Player.prototype.update = function (dt, vX) {
		...
		if (this.dying) {
			...
			this.dying -= 1 * dt;
			if (this.dying <= 0) {
				let player = new Mario.Player(level.playerPos);
				level.loader.call();
				input.reset();
			}
		}
	...
	Player.prototype.powerUp = function (idx) {
		...
		if (this.power === 0) {
			this.sprite.pos[0] = 80;
			const newy = this.sprite.pos[1] - 32;
