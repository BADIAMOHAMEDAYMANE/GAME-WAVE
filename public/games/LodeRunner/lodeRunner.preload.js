let RUNNER_SPEED = 0.35; // Adjusted for 60 FPS (approx half of 0.65 to maintain original real-time speed)
let DIG_SPEED = 0.4;
let FILL_SPEED = 0.15;
let GUARD_SPEED = 0.2;

const FLASH_SPEED = 0.25; //for flash cursor (hi-score input mode)

const COVER_PROGRESS_BAR_H = 32;
const COVER_PROGRESS_UNDER_Y = 116;
const COVER_RUNNER_UNDER_Y = 164;
const COVER_SIDE_X = 56;

//*********************
// preload cover page
//*********************
let coverBitmap, remakeBitmap;
let helpObj, helpBitmap, editHelpBitmap, demoHelpBitmap;
let mainMenuIconBitmap, mainMenuIconObj;
let selectIconBitmap, selectIconObj;
let checkBitmap;
let returnBitmap, select1Bitmap, nextBitmap;
let yesBitmap, noBitmap;
let coverPageLoad;

function showLoadingPage() {
	const coverPageImages = [
...
//BEGIN of preload		
//*****************************************************************************		
let preload; //preload resource object
let firstPlay = 0;

let runnerData, guardData;
let holeData, holeObj = {};
let textData;

let soundFall, soundDig, soundPass, soundEnding;

function preloadResource() {
	const runnerSprite = new createjs.Sprite(runnerData, "runRight");
	const progress = new createjs.Shape();
	const progressBorder = new createjs.Shape();
	const percentTxt = new createjs.Text("0", (COVER_PROGRESS_BAR_H * tileScale) + "px Arial", "#FF0000");
	...
	createjs.Ticker.setFPS(60); // Standardized to 60 FPS
	const preloadTicker = createjs.Ticker.on("tick", mainStage);
	...
	const width = canvas.width - 2 * COVER_SIDE_X * tileScale;
	const height = COVER_PROGRESS_BAR_H * tileScale;
	...
	function showRemakeBitmap() {
		const x = 372 * tileScale;
		const y = 128 * tileScale;
...
const curAiVersion = AI_VERSION;
function setSpeedByAiVersion() {
	const speedObj = spriteSpeed[curAiVersion - 1];
