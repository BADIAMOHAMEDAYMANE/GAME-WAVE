...
const INTRO_DURATION = 0.1;// In seconds
...
function CLASS_input() {
    ...
    function handle_keydown_global(evt) {
        ...
        if (evt.keyCode == KEY_CODE_LEFT || evt.keyCode == KEY_CODE_UP || evt.keyCode == KEY_CODE_RIGHT || evt.keyCode == KEY_CODE_DOWN) {
            evt.preventDefault();
        }
        ...
        if (!game.paused) {
            if (game.mode == 0) {
                ...
                if (game.wait_timer <= 0) {
                    game.load_level(1); // Skip intro/demo level 0, go to level 1
                }
            } else if (game.mode == 1) {
                if (game.wait_timer <= 0) {
                    if (game.level_ended == 0) {
